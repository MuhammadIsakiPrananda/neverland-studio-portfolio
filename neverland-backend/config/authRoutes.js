...
import { getSuccessRedirectUrlFromRequest, getErrorRedirectUrlFromRequest } from '../utils/urlConfig.js';
...
router.get(
  ...
  (req, res) => {
    ...
    try {
      ...
      const successRedirectUrl = getSuccessRedirectUrlFromRequest(req, token, publicUserData);
      res.redirect(successRedirectUrl);
    } catch (error) {
      logger.error('Error in Google OAuth callback handler.', { error: error.message, stack: error.stack });
      const errorRedirectUrl = getErrorRedirectUrlFromRequest(req, 'token_error', 'Failed to process authentication.');
