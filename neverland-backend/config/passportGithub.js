    ...
    new GithubStrategy(
      ...
      async (accessToken, refreshToken, profile, done) => {
        try {
          ...
          if (!email) {
            return done(new Error("Email tidak ditemukan di akun GitHub (Mungkin diset privat)."), false);
          }
          ...
          const user = await sequelize.transaction(async (t) => {
            ...
            if (existingUser) {
              ...
              console.log('✓ GitHub account linked/updated for existing user:', email);
              return existingUser;
            }

            // Buat User Baru jika belum ada
            let baseUsername = (profile.username || email.split('@')[0]).replace(/[^a-zA-Z0-9_]/g, "").slice(0, 20);
