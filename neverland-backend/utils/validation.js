...
export const validatePasswordStrength = (password, context = {}) => {
  ...
  // Disallow username or email in password
  if (context.username && password?.toLowerCase().includes(context.username.toLowerCase())) {
    errors.push('Password must not contain your username');
  }

  if (context.email) {
    const emailUserPart = context.email.split('@')[0];
    if (password?.toLowerCase().includes(emailUserPart.toLowerCase())) {
 ...
 * @returns {object} - { isValid, errors }
 */
export const validateRegistrationInput = ({ name, username, email, password }) => {
  const errors = {};
  ...
  if (!username || username.trim().length < 3) {
    ...
    errors.username = 'Username can only contain letters, numbers, underscores, and hyphens (3-30 characters)';
  }
...
export const validatePasswordChange = ({ currentPassword, newPassword }) => {
  ...
  const newPasswordValidation = validatePasswordStrength(newPassword, { username: req.user?.username });
