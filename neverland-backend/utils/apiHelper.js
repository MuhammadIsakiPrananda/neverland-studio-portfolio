...
export const getApiBaseUrl = () => {
  ...
  const apiUrl = import.meta.env.VITE_API_BASE_URL || process.env.NEXT_PUBLIC_API_URL;

  if (!apiUrl) {
    console.error("FATAL: API Base URL environment variable (e.g., VITE_API_BASE_URL) is not set.");
    // Fallback ke URL lokal jika tidak ada, untuk mencegah crash total saat development.
...
export const apiCall = async (endpoint, options = {}) => {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new ApiError(data?.msg || `Request failed with status ${response.status}`, response.status, data);
    }
    
    return data;
  } catch (error) {
    console.error(`API Call Failed [${finalOptions.method || 'GET'} ${endpoint}]:`, error);
    throw error;
  }
};
...
export const api = {
  ...
  patch: (endpoint, data, options) => apiCall(endpoint, { method: 'PATCH', body: JSON.stringify(data), ...options }),
