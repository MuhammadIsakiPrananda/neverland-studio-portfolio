...
export const getBackendUrl = () => {
  ...
  if (isProduction) {
    // In production, default to localhost if BACKEND_URL not set
    // This allows local testing even when NODE_ENV=production
    // Perbaikan: Di produksi, ini seharusnya menjadi error fatal atau peringatan yang sangat kuat.
    // Menggunakan localhost di produksi hampir pasti salah.
    logger.error('FATAL: BACKEND_URL is not configured in a production-like environment. This is a critical misconfiguration.');
    // Untuk mencegah crash, kita tetap fallback, tapi ini harus segera diperbaiki di server.
...
export const getFrontendUrl = () => {
  ...
  if (process.env.NODE_ENV === 'production') {
    logger.warn('FRONTEND_URL not configured in production. Falling back to default.');
...
export const detectFrontendOrigin = (req) => {
  ...
  if (xForwardedProto && xForwardedHost) {
    ...
    logger.debug('Detected frontend origin from X-Forwarded headers.', { origin });
...
export const isAllowedOrigin = (urlString) => {
  ...
  if (process.env.NODE_ENV === 'production' && !process.env.ALLOWED_REDIRECT_URIS) {
    logger.warn('SECURITY: ALLOWED_REDIRECT_URIS is not configured in production. OAuth redirects may fail.');
    return false;
  }
...
export const validateUrls = () => {
  ...
  if (nodeEnv === 'production') {
    const hasBackendUrl = process.env.BACKEND_URL && !process.env.BACKEND_URL.includes('localhost');
    const hasFrontendUrl = process.env.FRONTEND_URL && !process.env.FRONTEND_URL.includes('localhost');

    if (!hasBackendUrl) {
      logger.warn('CONFIG_WARNING: BACKEND_URL should be set to a public domain in production, not localhost.');
    }

    if (!hasFrontendUrl) {
      logger.warn('CONFIG_WARNING: FRONTEND_URL should be set to a public domain in production, not localhost.');
    }

    if (!process.env.JWT_SECRET) {
      logger.error('SECURITY_FATAL: JWT_SECRET is not set in production. Application will be insecure.');
    }
    
    if (!process.env.ALLOWED_REDIRECT_URIS) {
      logger.warn('CONFIG_WARNING: ALLOWED_REDIRECT_URIS should be configured in production for OAuth to work correctly.');
    }

    if (!process.env.GITHUB_CLIENT_ID || !process.env.GITHUB_CLIENT_SECRET) {
      logger.warn('CONFIG_WARNING: GitHub OAuth credentials are not configured for production.');
    }
  }
  logger.info('-------------------------');
  ...
  try {
    ...
    logger.error('Invalid URL format in environment variables.', { error: err.message });
 ...
 */
export const getSuccessRedirectUrl = (token, userData, frontendOrigin = null) => {
 ...
 */
export const getErrorRedirectUrl = (errorCode, details = '', frontendOrigin = null) => {
  const baseUrl = frontendOrigin || getFrontendUrl();
 ...
 * @returns {string} The full redirect URL.
 */
export const getErrorRedirectUrlFromRequest = (req, errorCode, details = '') => {
  const clientOrigin = detectFrontendOrigin(req);
...
export const getEnvironmentInfo = () => {
  ...
  return {
    ...
    hasGithubAuth: !!process.env.GITHUB_CLIENT_ID && !!process.env.GITHUB_CLIENT_SECRET,
