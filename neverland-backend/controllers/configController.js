...
export const getRecaptchaConfig = async (req, res, next) => {
  try {
    ...
    // Debugging log untuk di server console
    if (!siteKey) {
      console.error('❌ ERROR: RECAPTCHA_SITE_KEY is missing in environment variables!');
    ...
    const endpoints = {
      ...
      endpoints: {
        ...
        login: {
          method: 'POST',
          path: '/api/auth/login',
          description: 'Login dengan email/username dan password (reCAPTCHA tidak diperlukan)',
