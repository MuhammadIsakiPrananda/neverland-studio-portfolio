    ...
    <div className="w-full inline-flex flex-nowrap overflow-hidden [mask-image:_linear-gradient(to_right,transparent_0,_black_128px,_black_calc(100%-200px),transparent_100%)]">
      ...
      >
        {extendedPartners.map((partner, index) => (
          <li key={index}>
            <div
              className="flex items-center gap-4 text-slate-500 hover:text-amber-400 transition-colors duration-300 cursor-pointer"
              title={partner.name}
            >
              {partner.icon}
              <span className="text-xl font-semibold tracking-wider whitespace-nowrap">
                {partner.name}
              </span>
            </div>
          </li>
