...
export const AppearanceTabContent: React.FC<AppearanceTabContentProps> = ({ appSettings, onThemeChange, onSettingsChange }) => {
  ...
  const ToggleSwitch = ({ id, checked, onChange }: { id: string, checked: boolean, onChange: (e: React.ChangeEvent<HTMLInputElement>) => void }) => (
    <label htmlFor={id} className="relative inline-flex items-center cursor-pointer">
      <input type="checkbox" id={id} name={id} className="sr-only peer" checked={checked} onChange={onChange} />
      ...
      <div className="space-y-6">
        <div>
          ...
          <div className="relative flex w-full p-1 mt-2 bg-slate-800/60 rounded-lg border border-slate-700">
            <div
              className={`absolute top-1 bottom-1 left-1 w-[calc(50%-4px)] bg-slate-700/80 shadow-sm rounded-md transition-transform duration-300 ease-in-out ${appSettings.theme === 'dark' ? 'translate-x-full' : 'translate-x-0'}`}
            />
            <button onClick={() => onThemeChange('light')} className="relative z-10 flex-1 flex items-center justify-center gap-2 py-2 text-sm font-medium transition-colors">
                <Sun className={`w-5 h-5 transition-colors ${appSettings.theme === 'light' ? 'text-amber-400' : 'text-slate-400'}`} />
            ...
            <button onClick={() => onThemeChange('dark')} className="relative z-10 flex-1 flex items-center justify-center gap-2 py-2 text-sm font-medium transition-colors">
                <Moon className={`w-5 h-5 transition-colors ${appSettings.theme === 'dark' ? 'text-amber-400' : 'text-slate-400'}`} />
        ...
        <div>
          ...
          <div className="mt-2 divide-y divide-slate-800">
            <div className="flex items-center justify-between py-3">
              ...
              <ToggleSwitch id="emailNotifications" checked={appSettings.emailNotifications} onChange={onSettingsChange} />
            </div>
            <div className="flex items-center justify-between py-3">
              <p className="font-medium text-slate-200">Push Notifications</p>
              <ToggleSwitch id="pushNotifications" checked={appSettings.pushNotifications} onChange={onSettingsChange} />
            </div>
        </div>
        </div>
      </div>
