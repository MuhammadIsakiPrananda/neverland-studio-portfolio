...
import { Mail, Send, Loader, ShieldX, MailCheck, ArrowLeft } from 'lucide-react';
...
const ForgotPasswordForm: React.FC<ForgotPasswordFormProps> = ({ onSwitchMode }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      addNotification('Request Sent', `If an account exists for ${email}, you will receive a password reset link.`, 'success');
      setIsSubmitted(true);
    } catch (err: any) {
      ...
      {isSubmitted ? (
        ...
        <div key="form">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            {error && <div className="bg-red-900/40 border border-red-500/30 text-red-300 text-sm p-3 rounded-lg flex items-start gap-3 animate-shake"><ShieldX className="w-5 h-5 flex-shrink-0" /><span>{error}</span></div>}
            <div>
