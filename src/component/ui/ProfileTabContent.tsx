    ...
    <div className="space-y-8">
      ...
      >
        <div className="space-y-4">
          <div>
            ...
            <input type="text" name="name" value={formData.name} onChange={handleInputChange} className="w-full bg-slate-800/60 border border-slate-700 rounded-lg px-3 py-2 mt-1 focus:border-amber-500 focus:ring-2 focus:ring-amber-500/30 outline-none transition-colors" />
          </div>
          <div>
            <label className="text-sm font-medium text-slate-400">Username</label>
            <input type="text" name="username" value={formData.username} onChange={handleInputChange} className="w-full bg-slate-800/60 border border-slate-700 rounded-lg px-3 py-2 mt-1 focus:border-amber-500 focus:ring-2 focus:ring-amber-500/30 outline-none transition-colors" />
          </div>
          <div>
            <label className="text-sm font-medium text-slate-400">Bio</label>
            <textarea name="bio" value={formData.bio || ''} onChange={handleInputChange} rows={3} className="w-full bg-slate-800/60 border border-slate-700 rounded-lg px-3 py-2 mt-1 focus:border-amber-500 focus:ring-2 focus:ring-amber-500/30 outline-none transition-colors resize-none" placeholder="Tell us a little about yourself..."/>
          </div>
        </div>
      </SettingsCard>
      ...
      >
        <div>
          ...
          <div onClick={() => coverPhotoInputRef.current?.click()} className="mt-2 flex justify-center items-center w-full h-32 border-2 border-dashed border-slate-700 rounded-lg cursor-pointer hover:border-amber-500 transition-colors bg-slate-800/50">
