...
import { Loader, KeyRound, Copy, Trash2, PlusCircle, Check } from 'lucide-react';
...
export const ApiKeysTabContent: React.FC = () => {
  ...
  const generateRandomToken = () => `sk_live_${[...Array(32)].map(() => Math.random().toString(36)[2]).join('')}`;
  ...
  const handleGenerateKey = () => {
    ...
    setTimeout(() => {
      ...
      addNotification('API Key Generated', 'Your new key has been created. Copy it now, you won\'t see it again!', 'success');
    }, 1500);
  };
  ...
  const handleConfirmRevoke = () => {
    ...
    setTimeout(() => {
      ...
      addNotification('API Key Revoked', `The key "${keyToRevoke.name}" has been permanently deleted.`, 'warning');
      setIsRevoking(false);
      setKeyToRevoke(null);
    }, 1000);
  };
              ...
              >
                <div className="flex items-center gap-4 py-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-slate-400 font-mono">{getMaskedToken(key.token)}</p>
