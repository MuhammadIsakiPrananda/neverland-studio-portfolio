...
import { Download, CreditCard, Banknote, Wallet, Store, Save, Loader, Calendar, Lock, RefreshCw } from 'lucide-react';
import { SettingsCard } from './SettingsCard';

export const AccountTabContent: React.FC = () => {
  const paymentMethods = [
    { id: 'credit-card', name: 'Credit/Debit Card', icon: <CreditCard className="w-6 h-6 text-slate-400" /> },
    { id: 'virtual-account', name: 'Virtual Account', description: 'BCA, Mandiri, BNI, etc.', icon: <Banknote className="w-6 h-6 text-slate-400" /> },
    { id: 'e-wallet', name: 'E-Wallet', description: 'GoPay, OVO, DANA', icon: <Wallet className="w-6 h-6 text-slate-400" /> },
    { id: 'retail', name: 'Retail Outlet', description: 'Alfamart, Indomaret', icon: <Store className="w-6 h-6 text-slate-400" /> },
  ];
        ...
        <div className="mt-6 border-t border-slate-800 pt-6">
            {selectedPayment === 'credit-card' && (
                <div className="space-y-4 animate-fade-in">
                  <h4 className="text-md font-semibold text-white mb-2">Enter Card Details</h4>
                  <div>
            ...
            {selectedPayment !== 'credit-card' && (
              <div className="text-center text-slate-400 p-4 bg-slate-800/50 rounded-lg animate-fade-in">
                <p>Payment details and instructions will be provided upon saving changes.</p>
              </div>
            )}
        </div>
      </SettingsCard>
            ...
            {isHistoryLoading ? (
              ...
              [...Array(3)].map((_, index) => (
                <tr key={index} className="border-b border-slate-800 last:border-b-0">
                  <td className="p-3"><div className="h-4 bg-slate-700/50 rounded animate-pulse w-3/4"></div></td>
                  <td className="p-3"><div className="h-4 bg-slate-700/50 rounded animate-pulse w-1/2"></div></td>
                  <td className="p-3"><div className="h-4 bg-slate-700/50 rounded animate-pulse w-1/4"></div></td>
                  <td className="p-3 text-right"><div className="h-6 w-6 bg-slate-700/50 rounded-full animate-pulse ml-auto"></div></td>
                </tr>
              ))
            ) : invoices.length > 0 ? (
              // Actual Data
              invoices.map(invoice => (
                <tr key={invoice.id} className="border-b border-slate-800 text-sm last:border-b-0 hover:bg-slate-800/30 transition-colors">
              ...
              // Empty State
              <tr>
                <td colSpan={4} className="text-center p-8 text-slate-500">No billing history found.</td>
              </tr>
            )}
          </tbody>
        </table>
