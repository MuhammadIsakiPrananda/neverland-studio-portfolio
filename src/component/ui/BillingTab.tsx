...
const getCardIcon = (brand: string) => {
  ...
  if (brand.toLowerCase() === 'visa') return <img src="/visa-logo.svg" alt="Visa" className="w-10" />;
  if (brand.toLowerCase() === 'mastercard') return <img src="/mastercard-logo.svg" alt="Mastercard" className="w-10" />;
...
export const BillingTabContent: React.FC = () => {
  ...
  return (
    <div className="space-y-8">
        ...
        {!isAddingVA && (
          <ul className="space-y-4">
            {paymentMethods.map(pm => (
              <li key={pm.id} className="flex items-center justify-between p-4 bg-slate-800/50 border border-slate-700/50 rounded-lg">
                <div className="flex items-center gap-4">
                  {pm.type === 'card' ? getCardIcon(pm.brand) : <Landmark className="w-8 h-8 text-slate-400" />}
                  {pm.type === 'card' ? (
                    <div>
                      <p className="font-semibold text-white">{pm.brand} ending in {pm.last4}</p>
                      <p className="text-sm text-slate-400">Expires {pm.exp_month}/{pm.exp_year}</p>
                    </div>
                  ) : (
                    <div>
                      <p className="font-semibold text-white">{pm.brand} Virtual Account</p>
                      <p className="text-sm text-slate-400">Number ending in ••••{pm.last4}</p>
                    </div>
                  )}
      ...
      <SettingsCard title="Billing History" description="Review your past invoices and payments.">
        <ul className="divide-y divide-slate-800 -mx-6">
          {billingHistory.map(inv => (
            <li key={inv.id} className="flex items-center justify-between px-6 py-4">
              <div>
                <p className="font-semibold text-white">Invoice #{inv.id.slice(-6)}</p>
