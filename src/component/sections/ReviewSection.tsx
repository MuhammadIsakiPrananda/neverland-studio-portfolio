      ...
      <div className="container mx-auto px-4">
        ...
        >
          {testimonials.slice(0, 6).map((testimonial, index) => (
            ...
            >
              {/* Konten Card */}
              <figure className="relative h-full flex flex-col bg-slate-800/50 rounded-lg overflow-hidden border border-slate-700/50 transition-all duration-300 group-hover:border-slate-600 group-hover:shadow-2xl group-hover:shadow-cyan-500/10 group-hover:-translate-y-1">
                {/* Aksen Garis Gradien di Atas */}
                <div className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-teal-400 to-cyan-400 opacity-50 group-hover:opacity-100 transition-opacity duration-300"></div>
                ...
                {/* Informasi Penulis */}
                <figcaption className="flex items-center gap-4 p-6 border-t border-slate-700/50 bg-slate-900/20">
                  <img src={testimonial.image} alt={testimonial.name} className="w-12 h-12 rounded-full border-2 border-slate-600" />
        ...
        <div className="mt-16 flex justify-center">
          <button onClick={onWriteReviewClick} className="group relative inline-flex items-center justify-center gap-2.5 overflow-hidden rounded-lg bg-gradient-to-r from-teal-500 to-cyan-500 px-7 py-3 text-base font-semibold text-white shadow-lg shadow-cyan-500/20 transition-all duration-300 hover:-translate-y-0.5 hover:shadow-2xl hover:shadow-cyan-500/40">
