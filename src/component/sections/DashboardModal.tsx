...
import { X, User, Mail, Save, Loader, Shield, KeyRound, Smartphone, QrCode } from 'lucide-react';
...
const DashboardModal: React.FC<DashboardModalProps> = ({ isOpen, onClose, userProfile, onProfileUpdate }) => {
  ...
  const TabButton = ({ tab, currentTab, setTab, icon, children }: { tab: Tab, currentTab: Tab, setTab: (tab: Tab) => void, icon: ReactNode, children: ReactNode }) => (
    <button
      onClick={() => setTab(tab)}
          ...
          >
            ...
            <div className="w-1/4 bg-slate-900/50 border-r border-slate-800 p-6 flex flex-col">
              ...
              <div className="space-y-2">
                <TabButton tab="profile" currentTab={activeTab} setTab={setActiveTab} icon={<User className="w-5 h-5" />}>Profile</TabButton>
                <TabButton tab="security" currentTab={activeTab} setTab={setActiveTab} icon={<Shield className="w-5 h-5" />}>Security</TabButton>
              </div>
            ...
            <div className="w-3/4 p-8 overflow-y-auto">
              <AnimatePresence mode="wait">
                <motion.div key={activeTab} variants={contentVariants} initial="hidden" animate="visible" exit="exit">
                  {activeTab === 'profile' && (
                    <div>
                      <h3 className="text-2xl font-bold text-white mb-6">Public Profile</h3>
                      <form onSubmit={handleSubmit} className="space-y-6">
                        <div className="flex items-center gap-6">
                          <div className="relative w-24 h-24 rounded-full overflow-hidden border-2 border-slate-600 flex-shrink-0">
                            {avatar ? <img src={avatar} alt="Profile" className="w-full h-full object-cover" /> : <div className="w-full h-full bg-slate-700 flex items-center justify-center"><User className="w-12 h-12 text-slate-400" /></div>}
                          </div>
                          <div>
                            <input type="file" ref={fileInputRef} onChange={handleAvatarChange} accept="image/*" className="hidden" />
                            <button type="button" onClick={() => fileInputRef.current?.click()} className="px-4 py-2 bg-slate-700/50 border border-slate-600 rounded-lg text-sm font-medium hover:bg-slate-700 transition-colors">Change Photo</button>
                  ...
                  {activeTab === 'security' && (
                    <div>
                      ...
                      <div className="space-y-8">
                        ...
                        {/* 2FA Section */}
                        <div>
                          <h4 className="text-lg font-semibold text-slate-200 mb-4">Two-Factor Authentication</h4>
                          <div className="p-6 bg-slate-800/30 rounded-lg border border-slate-700/50">
                            {!isSettingUp2FA ? (
                              <div className="flex items-center justify-between">
                                <div>
                                  <p className="font-medium text-white">Status: <span className={is2faEnabled ? 'text-green-400' : 'text-red-400'}>{is2faEnabled ? 'Enabled' : 'Disabled'}</span></p>
                              ...
                              <AnimatePresence>
                                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                                  <p className="text-sm text-slate-300 mb-4">Scan the QR code with your authenticator app (e.g., Google Authenticator, Authy).</p>
