    ...
    >
      <div className="max-w-7xl mx-auto">
        ...
        <motion.div className="relative" variants={containerVariants}>
          {/* The connecting line */}
          <div className="absolute left-9 top-9 bottom-0 w-0.5 bg-slate-700/50 hidden md:block" aria-hidden="true"></div>
          
          <div className="space-y-12">
            {process.map((step, idx) => (
              <motion.div key={idx} variants={itemVariants} className="relative flex items-start gap-6 md:gap-8">
                {/* Step Number and Icon */}
                <div className="relative z-10 flex-shrink-0 flex flex-col items-center">
                  <div className="w-20 h-20 bg-slate-800 border-2 border-slate-700 rounded-full flex items-center justify-center text-teal-400 transition-colors group-hover:border-teal-500">
                    <span className="text-3xl">{step.icon}</span>
                  </div>
                  <div className="mt-2 text-xs font-bold uppercase tracking-wider text-slate-500">Step {step.step}</div>
                </div>
                {/* Content Card */}
                <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 flex-1 transition-all hover:border-teal-500/50 hover:shadow-2xl hover:shadow-teal-900/20">
