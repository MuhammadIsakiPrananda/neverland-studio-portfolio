...
const MobileNav: React.FC<MobileNavProps> = ({ isMenuOpen, setIsMenuOpen, activeSection, handleNavClick, isLoggedIn, userProfile, onLoginClick, onLogout, onDashboardClick }) => {
  ...
  if (pricingIndex !== -1) {
    navItems.splice(pricingIndex + 1, 0, { name: 'Reviews', href: '/reviews', icon: <MessageSquare className="w-5 h-5" /> });
  }
                    ...
                    <motion.li key={item.name} variants={menuItemVariants}>
                      ...
                      >
                        {activeSection === item.name && (
                          <motion.div layoutId="mobile-active-pill" className="absolute inset-0 bg-amber-500/10 rounded-lg" />
                        )}
                        ...
                        <span className={`relative flex items-center gap-4 transition-colors ${activeSection === item.name ? 'text-amber-400' : 'text-slate-300 group-hover:text-white'}`}>
                          {item.icon}
                          {item.name}
                        </span>
                      </button>
                  ...
                  {isLoggedIn ? (
                    <div className="flex items-center justify-center gap-3">
                      ...
                      <p className="text-sm text-slate-400">
                        Signed in as <span className="font-bold text-white">{userProfile?.name}</span>
                      </p>
                    </div>
                  ) : (
                    <button onClick={onLoginClick} className="w-full flex items-center justify-center gap-2.5 bg-gradient-to-r from-amber-500 to-orange-500 text-white px-6 py-3 rounded-full text-base font-semibold shadow-lg shadow-amber-500/20 hover:shadow-xl hover:shadow-amber-500/30 transition-all transform hover:scale-105">
