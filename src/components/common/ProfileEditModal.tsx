...
export default function ProfileEditModal({ theme, isOpen, onClose, currentProfile, onSave }: ProfileEditModalProps) {
  ...
  useEffect(() => {
    if (isOpen) {
      console.log('ProfileEditModal opened - Loading profile:', currentProfile);
      setProfile({
        name: '',
        email: '',
        phone: '',
      ...
      console.log('Avatar loaded:', currentProfile.avatar ? 'Yes ✅' : 'No ❌');
    }
  ...
  // Fetch active sessions - load once when section opens (no auto-refresh)
  useEffect(() => {
  ...
  // Handler to manually refresh sessions
  const handleRefreshSessions = async () => {
    try {
      ...
      if (response.success && response.data) {
        setActiveSessions(response.data.sessions || []);
        setLastSessionUpdate(new Date());
        showSuccess('Refreshed', 'Session list updated');
      }
    } catch (error: any) {
      showError('Refresh Failed', 'Failed to refresh sessions');
    } finally {
      setLoadingSessions(false);
    }
  };
      ...
      if (response.success) {
        ...
        try {
          ...
          if (freshProfile.success && freshProfile.data) {
            ...
            // Trigger callback to parent untuk update navbar dan state lainnya
            onSave(updatedProfile);

            // Dispatch custom event untuk trigger navbar update
          ...
          const fallbackProfile = {
            ...profile,
            username: response.data.user?.username || profile.email?.split('@')[0],
            role: response.data.user?.role || 'user'
          };
          onSave(fallbackProfile);
        ...
        const userName = response.data.user?.name || profile.name || 'User';
        showSuccess(
          'Profile Updated! ✨',
          `Synced with database: ${userName}!`
        );
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    console.log('Processing image for crop...', { fileName: file.name, fileSize: file.size });
  ...
  const handleCropComplete = (croppedImage: string) => {
    console.log('Image cropped successfully', {
      base64Length: croppedImage.length
    });

    // Update preview only - akan diupload saat klik Save Changes
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4">
      ...
      ">
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          `}>
            <h2 className={`text-xl sm:text-2xl font-bold ${isDark ? 'text-white' : 'text-stone-900'
              }`}>
              {sidebarSections.find(s => s.id === activeSection)?.label}
            </h2>
            <p className={`text-xs sm:text-sm mt-1 ${isDark ? 'text-gray-400' : 'text-stone-600'
              }`}>
              Update your {sidebarSections.find(s => s.id === activeSection)?.label.toLowerCase()}
            </p>
          </div>

          {/* Form Content - Scrollable with fixed buttons */}
          <form onSubmit={handleSubmit} className="relative z-10 flex-1 flex flex-col overflow-hidden">
            {/* Scrollable Content Area */}
            <div className="flex-1 overflow-y-auto scrollbar-hide px-3 py-4 sm:p-6">
              <div className="space-y-4 sm:space-y-6 max-w-2xl">
                ...
                {activeSection === 'profile' && (
                  ...
                  `}>
                      <h3 className={`text-base sm:text-lg font-semibold mb-3 sm:mb-4 ${isDark ? 'text-white' : 'text-stone-900'
                        }`}>
                        Profile Photo
                      </h3>
                      <div className="flex flex-col sm:flex-row items-center gap-4 sm:gap-6">
                        ...
                        <div className="flex-1 text-center sm:text-left w-full">
                          ...
                          <p className={`text-xs sm:text-sm mt-2 ${isDark ? 'text-gray-400' : 'text-stone-600'
                            }`}>
                  ...
                  `}>
                      ...
                      <div className="space-y-4 sm:space-y-5">
                        {/* Full Name */}
                        <div>
                          <label className={`block text-xs sm:text-sm font-medium mb-1.5 sm:mb-2 ${isDark ? 'text-gray-300' : 'text-stone-700'
                            }`}>
                            Full Name <span className="text-red-500">*</span>
                          </label>
                          <div className="relative">
                            <div className={`absolute left-3 sm:left-4 top-1/2 -translate-y-1/2 ${isDark ? 'text-gray-500' : 'text-stone-400'
                              }`}>
                          ...
                          <p className={`text-[10px] sm:text-xs mt-1 ${isDark ? 'text-gray-500' : 'text-stone-500'
                            }`}>
                        ...
                        <div>
                          ...
                          <div className="relative">
                            <div className={`absolute left-3 sm:left-4 top-1/2 -translate-y-1/2 ${isDark ? 'text-gray-600' : 'text-stone-500'
                              }`}>
                              <User className="w-4 h-4 sm:w-5 sm:h-5" />
                            </div>
                            <input
                          ...
                          <p className={`text-[10px] sm:text-xs mt-1 flex items-center gap-1 ${isDark ? 'text-gray-600' : 'text-stone-500'
                            }`}>
                      ...
                      <div className="space-y-4 sm:space-y-5">
                        ...
                        <div>
                          <div className="flex justify-between items-center mb-1.5 sm:mb-2">
                            <label className={`block text-xs sm:text-sm font-medium ${isDark ? 'text-gray-300' : 'text-stone-700'
                              }`}>
                              Bio / About Yourself
                            </label>
                            <span className={`text-[10px] sm:text-xs font-medium ${(profile.bio?.length || 0) > 500
                              ? 'text-red-500'
                          ...
                          <div className="relative">
                            <div className={`absolute left-3 sm:left-4 top-3 sm:top-4 ${isDark ? 'text-gray-500' : 'text-stone-400'
                              }`}>
                ...
                {activeSection === 'security' && (
                  <>
                    {/* Change Password */}
                    <div className={`p-6 rounded-2xl border ${isDark ? 'bg-gray-800/30 border-gray-700/50' : 'bg-white/50 border-stone-200'
                      }`}>
                      <h3 className={`text-lg font-semibold mb-2 flex items-center gap-2 ${isDark ? 'text-white' : 'text-stone-900'
                        }`}>
                      ...
                      {passwordErrorMessage && (
                        <div className={`mb-4 p-4 rounded-xl flex items-start gap-3 ${isDark ? 'bg-red-500/10 border border-red-500/20 text-red-300' : 'bg-red-50 border border-red-200 text-red-700'
                          }`}>
                      ...
                      {passwordSuccessMessage && (
                        <div className={`mb-4 p-4 rounded-xl ${isDark ? 'bg-green-500/10 border border-green-500/20 text-green-300' : 'bg-green-50 border border-green-200 text-green-700'
                          }`}>
                        ...
                        <div>
                          ...
                          <div className="relative">
                            ...
                            <button
                              ...
                              className={`absolute right-4 top-1/2 -translate-y-1/2 transition-colors ${isDark ? 'text-gray-500 hover:text-gray-300' : 'text-stone-400 hover:text-stone-600'
                                }`}
                            >
                          ...
                          {newPassword && (
                            <div className="mt-2 space-y-1">
                              <div className="flex items-center gap-2 text-xs">
                                <div className={`h-1 flex-1 rounded-full overflow-hidden ${isDark ? 'bg-gray-700' : 'bg-stone-200'
                                  }`}>
                                  <div
                                    className={`h-full transition-all duration-300 ${newPassword.length < 6 ? 'w-1/3 bg-red-500' :
                                      newPassword.length < 8 ? 'w-2/3 bg-yellow-500' :
                                        'w-full bg-green-500'
                                      }`}
                                  />
                                </div>
                                <span className={`font-medium ${newPassword.length < 6 ? 'text-red-500' :
                    ...
                    <div className={`p-6 rounded-xl border ${isDark ? 'bg-gray-800/30 border-gray-700/50' : 'bg-white border-stone-200'
                      }`}>
                      <h3 className={`text-lg font-semibold mb-2 flex items-center gap-2 ${isDark ? 'text-white' : 'text-stone-900'
                        }`}>
                      ...
                      {twoFASuccessMessage && (
                        <div className={`mb-4 p-3 rounded-lg ${isDark ? 'bg-green-500/20 border border-green-500/30 text-green-300' : 'bg-green-50 border border-green-200 text-green-700'
                          }`}>
                          {twoFASuccessMessage}
                        </div>
                      )}
                      {twoFAErrorMessage && (
                        <div className={`mb-4 p-3 rounded-lg ${isDark ? 'bg-red-500/20 border border-red-500/30 text-red-300' : 'bg-red-50 border border-red-200 text-red-700'
                          }`}>
                          {twoFAErrorMessage}
                        </div>
                      )}
                      ...
                      {isEnabling2FA && qrCodeData && (
                        <div className="mt-4">
                          ...
                          <div className={`text-xs text-center mb-4 flex items-center justify-center gap-2 ${isDark ? 'text-gray-500' : 'text-stone-500'
                            }`}>
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                            {/* QR Code Method */}
                            <div className={`p-4 rounded-xl border ${isDark ? 'bg-gray-800/30 border-gray-700/50' : 'bg-stone-50 border-stone-200'
                              }`}>
                            ...
                            <div className={`p-4 rounded-xl border ${isDark ? 'bg-gray-800/30 border-gray-700/50' : 'bg-stone-50 border-stone-200'
                              ...
                              <div className={`p-3 rounded-lg ${isDark ? 'bg-gray-900/50' : 'bg-white'
                                }`}>
                                <p className={`text-xs mb-2 text-center ${isDark ? 'text-gray-500' : 'text-stone-500'
                                  }`}>
                                  Secret Key:
                                </p>
                                <code className={`block text-center font-mono text-sm tracking-wider mb-3 break-all ${isDark ? 'text-blue-400' : 'text-blue-600'
                                  }`}>
                                  {twoFactorSecret}
                                </code>
                                <button
                                  ...
                                  className={`w-full px-3 py-2 rounded-lg text-sm font-medium transition-all flex items-center justify-center gap-2 ${isDark
                      ...
                      {showRecoveryCodes && recoveryCodes.length > 0 && (
                        <div className={`mt-4 p-4 rounded-xl border ${isDark ? 'bg-yellow-500/10 border-yellow-500/30' : 'bg-yellow-50 border-yellow-200'
                          }`}>
                          ...
                          <div className={`grid grid-cols-2 gap-2 mb-3 p-3 rounded-lg font-mono text-sm ${isDark ? 'bg-gray-900/50' : 'bg-white'
                            }`}>
                      ...
                      {showRecoveryCodes && recoveryCodes.length > 0 && (
                        <div className={`mt-4 p-4 rounded-lg ${isDark ? 'bg-amber-500/10 border border-amber-500/30' : 'bg-amber-50 border border-amber-200'
                          }`}>
                          ...
                          <div className="grid grid-cols-2 gap-2">
                            {recoveryCodes.map((code, index) => (
                              <code key={index} className={`text-xs p-2 rounded ${isDark ? 'bg-gray-900/50 text-amber-300' : 'bg-white text-amber-900'
                                }`}>
                                {code}
                              </code>
                            ))}
                          ...
                          <button
                            type="button"
                            onClick={() => setShowRecoveryCodes(false)}
                            className={`mt-3 w-full py-2 text-sm rounded-lg ${isDark ? 'bg-gray-800 text-gray-300 hover:bg-gray-700' : 'bg-stone-100 text-stone-700 hover:bg-stone-200'
                              }`}
                          >
                            I've saved my codes
                          </button>
                    ...
                    <div className={`p-6 rounded-xl border ${isDark ? 'bg-gray-800/30 border-gray-700/50' : 'bg-white border-stone-200'
                      ...
                      <h3 className={`text-lg font-semibold mb-4 flex items-center gap-2 ${isDark ? 'text-white' : 'text-stone-900'
                        }`}>
                      ...
                      <div className="space-y-3">
                        {loadingHistory ? (
                          // Loading skeleton
                          [1, 2, 3].map((item) => (
                            <div key={item} className={`p-3 rounded-lg animate-pulse ${isDark ? 'bg-gray-900/50' : 'bg-stone-50'
                              }`}>
                          ...
                          // Empty state
                          <div className={`p-6 text-center rounded-lg ${isDark ? 'bg-gray-900/30' : 'bg-stone-50'
                            }`}>
                            <Clock className={`w-12 h-12 mx-auto mb-2 ${isDark ? 'text-gray-600' : 'text-stone-400'
                              }`} />
                          ...
                          loginHistory.map((history) => (
                            <div key={history.id} className={`p-3 rounded-lg ${isDark ? 'bg-gray-900/50' : 'bg-stone-50'
                              }`}>
                              <div className="flex justify-between items-start mb-1">
                                <span className={`font-medium ${isDark ? 'text-white' : 'text-stone-900'}`}>
                                  {history.platform} · {history.browser}
                                </span>
                                <span className={`text-xs px-2 py-1 rounded ${history.status === 'success'
                    ...
                    <div className={`p-6 rounded-xl border ${isDark ? 'bg-gray-800/30 border-gray-700/50' : 'bg-white border-stone-200'
                      ...
                      <div className="flex items-center justify-between mb-4">
                        <h3 className={`text-lg font-semibold flex items-center gap-2 ${isDark ? 'text-white' : 'text-stone-900'
                          }`}>
                          <Monitor className="w-5 h-5" />
                          Active Sessions
                        </h3>
                        <div className="flex items-center gap-3">
                          {lastSessionUpdate && !loadingSessions && (
                            <span className={`text-xs ${isDark ? 'text-gray-500' : 'text-stone-500'}`}>
                              Updated {formatRelativeTime(lastSessionUpdate.toISOString())}
                            </span>
                          )}
                          <button
                            type="button"
                            ...
                            className={`
                              ...
                              ${loadingSessions ? 'opacity-50 cursor-not-allowed' : ''}
                            `}
                      ...
                      <div className="space-y-3">
                        {loadingSessions ? (
                          ...
                          <div className={`p-6 text-center rounded-lg ${isDark ? 'bg-gray-900/30' : 'bg-stone-50'
                            ...
                            <Monitor className={`w-12 h-12 mx-auto mb-2 ${isDark ? 'text-gray-600' : 'text-stone-400'
                              }`} />
                            <p className={`text-sm ${isDark ? 'text-gray-400' : 'text-stone-600'}`}>
                              No active sessions
                            </p>
                          ...
                          activeSessions.map((session) => (
                            <div key={session.id} className={`p-3 rounded-lg ${isDark ? 'bg-gray-900/50' : 'bg-stone-50'
                              }`}>
                    ...
                    <div className={`p-6 rounded-2xl border ${isDark ? 'bg-red-900/20 border-red-500/30' : 'bg-red-50 border-red-200'
                      ...
                      <h3 className={`text-lg font-semibold mb-2 flex items-center gap-2 ${isDark ? 'text-red-400' : 'text-red-700'
                        }`}>
                      ...
                      <div className={`p-4 rounded-xl mb-4 ${isDark ? 'bg-red-950/30 border border-red-500/20' : 'bg-red-100/50 border border-red-300'
                        }`}>
                        <div className="flex items-start gap-3">
                          ...
                          <div>
                            ...
                            <ul className={`text-sm space-y-1 ${isDark ? 'text-red-300/80' : 'text-red-600/80'
                              }`}>
                ...
                {(successMessage || errorMessage) && (
                  <div className="space-y-3">
                    {successMessage && (
                      <div className={`p-4 rounded-xl border ${isDark ? 'bg-green-500/20 border-green-500/30 text-green-300' : 'bg-green-50 border-green-200 text-green-700'
                        }`}>
                    ...
                    {errorMessage && (
                      <div className={`p-4 rounded-xl border ${isDark ? 'bg-red-500/20 border-red-500/30 text-red-300' : 'bg-red-50 border-red-200 text-red-700'
                        }`}>
                        <div className="flex items-center gap-2">
                          <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            {activeSection !== 'security' && (
              <div className={`
                flex-shrink-0 border-t px-3 py-4 sm:px-6 sm:py-4
                ${isDark ? 'bg-slate-900/95 border-slate-700/50' : 'bg-white/95 border-stone-200'}
                backdrop-blur-sm
              `}>
                <div className="flex flex-col-reverse sm:flex-row gap-3 max-w-2xl">
                  <button
                    type="button"
                    onClick={onClose}
                    disabled={loading}
      ...
      {showDisable2FAModal && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
          ...
          {/* Modal */}
          <div className={`relative w-full max-w-md rounded-2xl shadow-2xl ${isDark ? 'bg-gray-900 border border-gray-800' : 'bg-white border border-stone-200'
            }`}>
            {/* Header */}
            ...
            <div className="p-6 space-y-4">
              ...
              {/* Error Message */}
              {errorMessage && (
                <div className={`p-3 rounded-lg border ${isDark ? 'bg-red-500/20 border-red-500/30 text-red-300' : 'bg-red-50 border-red-200 text-red-700'
                  }`}>
                  {errorMessage}
                </div>
              )}
          ...
          `}>
            ...
            <div className="p-6 space-y-5">
              ...
              <div className={`p-4 rounded-xl ${isDark ? 'bg-red-500/5 border border-red-500/20' : 'bg-red-50 border border-red-200'
                ...
                <p className={`text-sm font-medium mb-3 ${isDark ? 'text-red-400' : 'text-red-700'
                  }`}>
                  The following will be permanently deleted:
                </p>
                <ul className={`text-sm space-y-2 ${isDark ? 'text-gray-400' : 'text-stone-600'
                  }`}>
              ...
              {/* Error Message */}
              {errorMessage && (
                <div className={`p-3 rounded-lg flex items-center gap-2 text-sm ${isDark ? 'bg-red-500/10 text-red-400' : 'bg-red-50 text-red-700'
                  }`}>
              ...
              <div>
                <label className={`block text-sm font-medium mb-2 ${isDark ? 'text-gray-300' : 'text-stone-700'
                  ...
                  Type <span className={`font-mono px-1.5 py-0.5 rounded ${isDark ? 'bg-gray-800 text-red-400' : 'bg-stone-100 text-red-600'
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <button
                    ...
                    className={`absolute right-3 top-1/2 -translate-y-1/2 ${isDark ? 'text-gray-500 hover:text-gray-300' : 'text-stone-400 hover:text-stone-600'
                      }`}
                  >
