...
// --- Fungsi Simulasi ---
const projectTypes: Collaboration["projectType"][] = [
  "Web Development",
  "Mobile App",
  "Branding",
  "Consultation",
...
const generateRandomCollaboration = (): Collaboration => {
  ...
  return {
    ...
    companyName,
    contactPerson,
    email: `${contactPerson.toLowerCase().replace(" ", ".")}@${companyName
      .toLowerCase()
    ...
    status: "New",
    message:
      "We are interested in discussing a potential project. Please let us know your availability for a brief call.",
  };
};

const initialCollaborations: Collaboration[] = Array.from({ length: 4 }).map(
  generateRandomCollaboration
);
...
const Collaborations = () => {
  ...
  const statusStyles: Record<Collaboration["status"], string> = {
    ...
    Contacted: "bg-fresh-coral-500/20 text-fresh-coral-400",
            ...
            <tbody>
              {collaborations.map((collab) => (
                ...
                >
                  ...
                  <td className="py-4 px-6">
                    {collab.submittedAt.toLocaleDateString()}
                  </td>
                  <td className="py-4 px-6">
                    <span
                  ...
                  <td className="py-4 px-6">
                    <div className="flex items-center justify-center gap-3">
                      ...
                      <button
                        className="p-2 text-slate-400 hover:text-sky-400 hover:bg-slate-700 rounded-full transition-colors"
                        title="Mark as Contacted"
                      >
                        <Phone className="w-4 h-4" />
                      </button>
                      <button
                        className="p-2 text-slate-400 hover:text-amber-400 hover:bg-slate-700 rounded-full transition-colors"
                        title="Start Project"
                      >
