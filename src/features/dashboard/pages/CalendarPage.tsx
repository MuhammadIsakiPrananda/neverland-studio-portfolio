      ...
      <div className="bg-slate-800/50 border border-slate-700 rounded-xl shadow-lg overflow-hidden">
        ...
        <div className="flex items-center justify-between p-4 border-b border-slate-700">
          ...
          <div className="hidden sm:block text-right">
            <div className="text-3xl font-mono text-fresh-mint-400">
        ...
        <div className="grid grid-cols-7">
          ...
          {dayNames.map((dayName) => (
            <div
              key={dayName}
              className="text-center py-3 text-xs font-bold uppercase text-slate-400 border-b border-r border-slate-700"
            >
              {dayName}
            </div>
          ))}
          ...
          {days.map((day, index) => {
            ...
            const dateNumberClasses = [
              ...
              isToday && "bg-fresh-mint-500 text-white font-bold shadow-lg",
            ...
            return (
              ...
              >
                <div className={dateNumberClasses}>{format(day, "d")}</div>
                {/* Area ini bisa diisi dengan event kalender */}
                <div className="mt-8 text-xs">
                  {/* Contoh event */}
                  {isToday && (
                    <div className="flex items-center gap-1.5 bg-fresh-mint-500/20 text-fresh-mint-300 p-1 rounded-md">
                      <div className="w-1.5 h-1.5 bg-fresh-mint-400 rounded-full"></div>
