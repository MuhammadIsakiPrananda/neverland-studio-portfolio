      ...
      <div className="grid gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 mb-6">
        {kpiData.length > 0
          ...
          : Array.from({ length: 4 }).map((_, index) => (
              <div key={index} className="bg-zinc-900/50 border border-zinc-800 p-5 rounded-xl animate-pulse">
        ...
        <ChartCard title="Page Views (Last 30 Mins)" className="lg:col-span-4">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={analytics?.pageViewsHistory}>
              ...
              <YAxis stroke="#a1a1aa" />
              <Tooltip contentStyle={{ backgroundColor: 'rgba(24, 24, 27, 0.9)', borderColor: '#27272a' }} cursor={{ fill: 'rgba(251, 191, 36, 0.1)' }} />
        ...
        <ChartCard title="Device Breakdown" className="lg:col-span-1">
          <ResponsiveContainer width="100%" height="100%">
            <PieChart>
              ...
              <Tooltip contentStyle={{ backgroundColor: 'rgba(24, 24, 27, 0.9)', borderColor: '#27272a' }} />
        ...
        <ChartCard title="Top Pages" className="lg:col-span-2 h-96">
          <div className="w-full h-full overflow-y-auto pr-2">
            <table className="w-full text-sm text-left">
              <thead className="text-xs text-zinc-400 uppercase sticky top-0 bg-zinc-900/50 backdrop-blur-sm">
                <tr>
              ...
              <tbody>
                {analytics?.topPages.map((page) => (
                  <tr key={page.path} className="border-t border-zinc-800">
                    <td className="py-3 px-4 font-medium text-zinc-300 flex items-center gap-2">
        ...
        <ChartCard title="Top Referrers" className="lg:col-span-2 h-96">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={analytics?.referrers} layout="vertical">
              ...
              <YAxis type="category" dataKey="name" stroke="#a1a1aa" width={80} tick={{ fill: '#d4d4d8' }} tickLine={false} axisLine={false} />
              <Tooltip contentStyle={{ backgroundColor: 'rgba(24, 24, 27, 0.9)', borderColor: '#27272a' }} cursor={{ fill: 'rgba(251, 191, 36, 0.1)' }} />
                ...
                {analytics?.liveEvents.map((event) => (
                  <li key={event.id} className="flex items-start gap-3">
                    ...
                    <div className="text-zinc-400">
                      A visitor from <span className="font-semibold text-zinc-200">{event.city}</span> {event.action} <a href="#" className="font-semibold text-amber-400 hover:underline">{event.target}</a>
                    </div>
                  </li>
                ))}
              </ul>
