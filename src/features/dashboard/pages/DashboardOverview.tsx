...
const fetchFullSimulatedMetrics = async (): Promise<ServerMetrics> => {
  ...
  const cpuHistory = Array.from({ length: 60 }).map((_, i) => ({
    time: new Date(now.getTime() - (60 - i) * 1000).toLocaleTimeString([], {
      hour: "2-digit",
      minute: "2-digit",
      second: "2-digit",
    }),
  ...
  const topProcesses: TopProcess[] = [
    ...
    },
    {
      ...
      mem: 4.1,
      command: "postgres: 13/main",
    },
    {
      pid: 2345,
      user: "root",
      cpu: parseFloat((Math.random() * 5).toFixed(2)),
      mem: 1.2,
    ...
    { pid: 1, user: "root", cpu: 0.1, mem: 0.8, command: "/sbin/init" },
    {
      ...
      mem: 0.9,
      command: "sshd: isaki@pts/0",
    },
  ].sort((a, b) => b.cpu - a.cpu);
  const suspiciousActivities: SuspiciousActivity[] = [
    {
      id: 1,
      timestamp: "2024-05-23 10:15:02",
      level: "Critical",
      sourceIp: "185.191.171.12",
      event: "Failed SSH login (user: root)",
    },
    {
      id: 2,
      timestamp: "2024-05-23 10:14:58",
      level: "Warning",
      sourceIp: "45.9.20.78",
      event: "Port scan detected on port 22, 80, 443",
    },
    {
      id: 3,
      timestamp: "2024-05-23 09:55:10",
      level: "Critical",
      sourceIp: "185.191.171.12",
      event: "Failed SSH login (user: root)",
    },
    {
      id: 4,
      timestamp: "2024-05-23 09:40:21",
      level: "Info",
      sourceIp: "103.224.182.242",
      event: "Anomalous traffic pattern from known botnet",
    },
    {
      id: 5,
      timestamp: "2024-05-23 09:30:05",
      level: "Warning",
      sourceIp: "192.168.1.105",
      event: "Multiple 404 errors from internal IP",
    },
  ];
  ...
  const kpiData = useMemo(() => {
    ...
    return [
      ...
      },
      {
        ...
        color: "text-premium-champagne-gold-400",
      },
      {
        title: "Disk Usage",
      ...
      },
      {
        ...
        value: (
          <div className="flex items-center gap-2 text-base font-semibold">
            <div className="flex items-center gap-1.5 p-1.5 rounded-md bg-fresh-mint-900/50 text-fresh-mint-400">
        ...
        color: "text-fresh-mint-400", // Warna dominan
      },
      {
        title: "Load Avg (1m)",
  ...
  return (
    <div className="p-2 sm:p-6">
      <div className="grid gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 mb-6">
        {kpiData.length > 0
          ? kpiData.map((kpi) => (
              <KpiCard key={kpi.title} {...kpi} value={kpi.value as any} />
            ))
          : Array.from({ length: 7 }).map((_, index) => (
              <div
                key={index}
                className="bg-slate-800/50 border border-slate-700 p-5 rounded-xl animate-pulse"
              >
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6 mb-6">
        <div className="xl:col-span-2">
          {metrics ? (
            <CpuUsageChart data={metrics.cpuHistory} />
          ) : (
            <div className="bg-slate-800/50 border border-slate-700 p-4 rounded-xl h-80 animate-pulse"></div>
          )}
        </div>
        <div>
          {metrics ? (
        ...
        <div className="xl:col-span-2">
          {metrics ? (
            <NetworkUsageChart data={metrics.networkHistory} />
          ) : (
            <div className="bg-slate-800/50 border border-slate-700 p-4 rounded-xl h-80 animate-pulse"></div>
          )}
        </div>
        <div>
          {metrics ? (
            <TopProcessesPanel processes={metrics.topProcesses} />
          ) : (
            <div className="bg-slate-800/50 border border-slate-700 p-4 rounded-xl h-80 animate-pulse"></div>
          )}
        </div>
      ...
      {metrics ? (
        <SuspiciousActivityLog activities={metrics.suspiciousActivities} />
      ) : (
        <div className="bg-slate-800/50 border border-slate-700 p-4 rounded-xl h-96 animate-pulse"></div>
      )}
    </div>
  );
};
