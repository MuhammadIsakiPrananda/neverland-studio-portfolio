  ...
  <div className="bg-slate-800/50 border border-slate-700 p-4 rounded-xl h-80">
    <div className="flex justify-between items-center mb-4">
      ...
      <div className="flex items-center gap-4 text-sm">
        <div className="flex items-center gap-2"><div className="w-2.5 h-2.5 rounded-full bg-teal-400"></div><span className="text-slate-300">In</span></div>
        <div className="flex items-center gap-2"><div className="w-2.5 h-2.5 rounded-full bg-rose-400"></div><span className="text-slate-300">Out</span></div>
      </div>
    </div>
    <ResponsiveContainer width="100%" height="100%">
      <AreaChart data={data} margin={{ top: 5, right: 20, left: -10, bottom: 20 }}>
        ...
        <Area type="monotone" dataKey="in" name="In" unit="KB/s" stroke="#2dd4bf" fill="url(#netInGradient)" strokeWidth={2} activeDot={{ r: 6, strokeWidth: 2, stroke: '#fff' }} dot={false} />
        <Area type="monotone" dataKey="out" name="Out" unit="KB/s" stroke="#fb7185" fill="url(#netOutGradient)" strokeWidth={2} activeDot={{ r: 6, strokeWidth: 2, stroke: '#fff' }} dot={false} />
      </AreaChart>
    </ResponsiveContainer>
  </div>
);