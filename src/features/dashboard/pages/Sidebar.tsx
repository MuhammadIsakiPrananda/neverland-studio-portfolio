...
const navigation: NavItemType[] = [
  ...
  { name: "Calendar", path: "/dashboard/calendar", icon: FiCalendar },
  {
    ...
    children: [
      ...
      {
        name: "Applicants",
        path: "/dashboard/inbox/applicants",
        icon: FiUsers,
        badge: 5,
      },
      {
        name: "Collaborations",
        path: "/dashboard/inbox/collaborations",
        icon: FiBriefcase,
      },
    ],
  },
...
const Badge = ({ value }: { value: number | boolean }) => {
  if (value === true) {
    return (
      <span className="w-2 h-2 bg-premium-champagne-gold-400 rounded-full animate-pulse" />
    );
  }
    ...
    <>
      ...
      <AnimatePresence mode="wait">
        <motion.span
          ...
          className={`relative flex-1 font-medium text-sm truncate ${
      ...
      {hasChildren && (
        <FiChevronDown
          className={`relative w-4 h-4 text-slate-400 transition-transform duration-200 ${
            isOpen ? "rotate-180" : ""
          }`}
        />
      )}
