...
const statusColors: Record<ServiceStatus, string> = {
  Active: "bg-premium-champagne-gold-500",
...
export const ServiceRow = ({ name, status }: ServiceRowProps) => (
  <div className="flex flex-col items-center justify-center text-center p-2 rounded-lg bg-slate-700/50 hover:bg-slate-700/80 transition-colors">
    <div className="relative">
      ...
      <div
        className={`absolute -top-1 -right-1 w-2.5 h-2.5 rounded-full border-2 border-slate-800 ${statusColors[status]}`}
      ></div>
    </div>
    <span className="text-xs text-slate-300 mt-2 truncate w-full" title={name}>
      {name}
    </span>
  </div>
);
