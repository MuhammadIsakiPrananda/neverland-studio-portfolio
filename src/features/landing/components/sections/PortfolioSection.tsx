...
import { portfolio } from "@/features/landing/data/portfolio";
...
const PortfolioSection: React.FC<PortfolioSectionProps> = ({
  ...
  const getCategoryTranslation = (category: string) => {
    const categoryMap: Record<string, string> = {
      ...
      Mobile: t("portfolio.filters.mobile"),
      Design: t("portfolio.filters.design"),
      Video: t("portfolio.filters.video"),
      Photography: t("portfolio.filters.photography"),
      Animation: t("portfolio.filters.animation"),
    };
      ...
      <div className="max-w-7xl mx-auto">
          ...
          <AnimatePresence>
            {filteredPortfolio.slice(0, visibleCount).map((project) => (
              <motion.div
                ...
                exit="exit"
                onClick={() =>
                  (project as Project).videoUrl
                    ? setShowVideo(true)
                    : setSelectedProject(project)
                }
                className="group relative bg-slate-900/50 rounded-2xl overflow-hidden border border-white/10 hover:border-amber-500/50 transition-all cursor-pointer"
              >
                ...
                <div className="p-6">
                  ...
                  <div className="flex flex-wrap gap-2 mb-4">
                    {project.tags.map((tag: string, i: number) => (
                      <span
                        key={i}
                        className="text-xs bg-slate-800/50 px-3 py-1 rounded-full text-slate-300"
                      >
                        {tag}
                      </span>
                    ))}
        ...
        <div className="text-center mt-12 flex gap-4 justify-center">
          ...
          <button
            ...
            className="bg-gradient-to-r from-amber-400 to-amber-600 text-white px-8 py-4 rounded-full font-semibold shadow-lg shadow-amber-500/20 hover:shadow-xl hover:shadow-amber-500/30 transition-all transform hover:scale-105"
          >
                ...
                <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
                  ...
                  <div className="lg:col-span-3">
                    ...
                    <p className="text-slate-300 leading-relaxed mb-6">
                      {selectedProject.desc} This is a more detailed description
                      for the case study. We can elaborate on the challenges,
                      our approach, the technologies used, and the final outcome
                      of the project. For this example, we're just using
                      placeholder text to demonstrate a more complete layout.
                    </p>
                    ...
                    <div className="flex flex-wrap gap-2 mb-8">
                      {selectedProject.tags.map((tag: string, i: number) => (
                        <span
                          key={i}
                          className="text-xs bg-slate-700/80 px-3 py-1.5 rounded-full text-slate-300 font-medium"
                        >
                          {tag}
                        </span>
                      ))}
                    ...
                    <button className="w-full bg-gradient-to-r from-amber-400 to-amber-600 text-white px-6 py-3 rounded-lg font-semibold shadow-lg shadow-amber-500/20 hover:shadow-xl hover:shadow-amber-500/30 transition-all flex items-center justify-center gap-2.5 group transform hover:scale-105">
                      Visit Live Site
