      ...
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-16">
          <div className="flex justify-center mb-6">
            <div className="relative inline-flex items-center gap-2 overflow-hidden rounded-full bg-slate-800/50 px-4 py-1.5 text-sm font-medium border border-amber-500/30 shadow-lg shadow-amber-900/50">
              <div className="absolute inset-0 bg-gradient-to-r from-amber-500/10 to-amber-600/10 opacity-60 blur-lg animate-pulse-slow"></div>
              <Star className="relative w-4 h-4 text-amber-300" />
              <span className="relative bg-gradient-to-r from-amber-300 to-amber-500 bg-clip-text font-semibold text-transparent">
                Testimonials
              </span>
            </div>
          </div>
        ...
        >
          {testimonials.map((testimonial, idx) => (
            <SpotlightCard
              key={idx}
              className="bg-gradient-to-br from-slate-900/80 to-black/80 backdrop-blur-lg border border-white/10 rounded-2xl p-8 transition-all flex flex-col group transform hover:-translate-y-2 hover:border-amber-500/50"
            >
              <motion.div
                whileHover={{ y: -4 }}
